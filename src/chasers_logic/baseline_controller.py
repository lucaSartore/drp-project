...
class BaselineController(IController):
    ...
    @classmethod
    def build(cls, number_of_agents: int, agent_id: int, settings: Settings) -> Self: #type: ignore
        return cls(agent_id)
    ...
    def control_loop(self, map: Map):
        ...
        elif (self.objective - position).abs() <= BaselineController.NEW_OBJ_TH:

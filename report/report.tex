...
\subsection{Proposed methods performances}
...
\begin{itemize}
    ...
    \item \textbf{TTFC (or Time To First Contact):} this is the total (in simulations steps)
    that past from the beginning from when the runner entered the detection radius of a chaser
    \item \textbf{TChase:} defined as $TTC - TTFC$
\end{itemize}

The figure \ref{fig:comparison} show the three metrics across all three approaches, with the error bar
...
The table below show the t-test's P-value of the comparison between 
the various metrics and solutions.

We can easily see from the table that, even tho the variance is pretty high
thanks to the high number of samples we can say with high confidence that all
the difference shown are statistically significant.
\begin{table}[ht]
\centering
...
\paragraph{Expleination of the observed results:}
Two interesting trends are clear when looking at figure \ref{fig:comparison}.
The first one is that the gaussian controller performs really poorly, even worst than the
simple baseline one; This can be explained with three key reasons:

\begin{itemize}
    \item The gaussian is not really suited for the search process, it can't provide a reasonable approximation
    on where the runner is making the search ineffective. In particular if the variance is too low the search process
    is focussed too much on the center of the map, and if the variance is too high most of the probability density
    ...
    and it also made it more susceptible to false positive measures. (given than, once a false positive measure occurs, the
    associated information can't be undone by a subsequent negative measure)
    \item The fact that we don't keep the direction in the state of the system make the tracking process slower (as at every
    iteration the gaussian has simply is variance increase, and then is conditioned based on the measures).
    This make it so that the center of the gaussian is always a bit behind the robot's position, and this causes the
    chasing process take a bit longer.
\end{itemize}

Observing the comparison between the ``Baseline'' and the ``PF'' solution reveal some more interesting threads:
the difference in TChase is really small at about 12\% while the difference in TTFC is much higher at approximately
107\%; This suggest that the main benefits of using an informed search strategy is not really the tracking of the target once
it has been spotted, but it has more to do with the ability of searching the target in an efficient way even before
the target has been spotted once.

This effect can be seen in figure \ref{fig:pf_map} where the chasers have an somewhat accurate 
guess on the position of the runner, even if none of them has yet being able to see it.

\begin{figure}[ht]
    \centering
    \includegraphics[width=0.4\textwidth]{./images/pf_map.png}
    \caption{Chasers guess of the runner's position (left image show the map with runners in red an chaser in green, right image 
...
The figure XXX show the computational cost of the proposed methods.
...
to improve the performances if we for example have a limited budget for sensors.
...
I should mentioned that the run to run variance on this is relatively high
at about 60 points on average.
The results have being calculated with the same 100 fix-seed tests
used for the previous benchmarks, however fixing the seed only
makes the initial condition the same, and doesn't have influence on how the
system evolves.
Unfortunately increasing sample size and reducing the variance even further
...
seem to be the detection radius, with all other variable having a smaller
(although still measurable) impact.

We can also see how some time a variable's effect seem to have a non monotonic effect on the 
time to catch (such as in the case of the false negative rate); However as we sed earlier
this test have some high variance associated to them, so the correct way to interpret
this result is that the effect at a certain point is quite small, and have a
impact that is comparably small
...
in a runner-chasers scenario.

The results have shown that the biggest gains can be had in the first part of
an episode (the search) rather than the second part (the chase), this
resulted in methods that can effectively represent complex PDF (namely the particle
filter) greatly outperforming other methods.

The work done still has some limitations, in particular
the way the probability of false positive has being approximated could be
...
the position) could further improve the performances of the algorithms

These proposed improvements are a grate opportunity for potential future works
related to this project, however the main challenge still remain the computational
cost of techniques such as the particle filter, and this challenges would only
increase if we started tracking fake runners, or adding components to the state.

Chaser's coordination is also something that was not threated in this report,
